...
if(isset($_GET['email_id']) && isset($_GET['loadSent']))
  {
...
$selectedEmail = $inbox_obj -> selectedEmail($email_id);
	...
	$inbox_result  = $inbox_obj -> select_emails($emails_sent);
        while($row = mysql_fetch_array ($inbox_result))
        {
          ...
          <div class=\"msg\" id=\"{$row['uid']}\">
              ...
              <form class=\"email-heading {$row['status']}\" target=\"{$emails_sent}\" action=\"message_body.php\" method=\"get\">
              <button type=\"submit\" class=\"email-link\" name=\"action\" value=\"show_msg\">
                    <input class=\"disable\" type=\"text\" name=\"uid\" value=\"{$row['uid']}\">
                    <input class=\"disable\" type=\"text\" name=\"email_inbox\" value=\"{$emails_sent}\">
                    <input class=\"email_from\" type=\"text\" name=\"email_from\" placeholder=\"{$row['email_to']}\" disabled>
