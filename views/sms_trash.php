...
if(isset($_GET['tel_id']) && isset($_GET['loadTrash']))
  {
...
$inbox_sms  = $inbox_obj -> select_sms($sms_trash);
        while($row = mysql_fetch_array ($inbox_sms))
        {
          ...
          <div class=\"msg\" id=\"{$row['id']}\">
              ...
              <form class=\"email-heading {$row['status']}\" target=\"{$sms_trash}\" action=\"sms_body.php\" method=\"get\">
              <button type=\"submit\" class=\"email-link\" name=\"action\" value=\"show_sms\">
                    <input class=\"disable\" type=\"text\" name=\"id\" value=\"{$row['id']}\">
                    <input class=\"disable\" type=\"text\" name=\"sms_inbox\" value=\"{$sms_trash}\">
